...
from main_win.win_old_people import Ui_mainWindow
...
#### 初始化睡眠监测模型
from hook import sleep
## 表情识别
from hook import emotion
            ...
            if self.is_continue:
                ...
                for i, det in enumerate(pred):  # detections per image
                    ...
                    if det is not None and len(det):
                        ...
                        # 睡眠监测
                        if self.issleep:
                            frame,content=sleep.sleep_detect(im0)
                            cv2.putText(im0, content, (20, 50), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 2)


                        if self.isemotion:
                            pre_res=emotion.detect_emotion(im0)
...
class MainWindow(QMainWindow, Ui_mainWindow):
    def __init__(self, parent=None):
        ...
        # 新增模型算法选择
        self.fallDetectBox.clicked.connect(self.is_fall_dect)
        self.standBOX.clicked.connect(self.is_sleep)
        self.emotionRecBox.clicked.connect(self.is_emotion)
    ...
    # add 跌倒检测
    def is_fall_dect(self):
        if self.fallDetectBox.isChecked():
            # 选中时
            self.det_thread.is_fall = True
        else:
            self.det_thread.is_fall = False
    #  add sleep
    def is_sleep(self):
        if self.standBOX.isChecked():
            # 选中时
            self.det_thread.issleep = True
        else:
            self.det_thread.issleep = False

    def is_emotion(self):
        if self.emotionRecBox.isChecked():
            # 选中时
            self.det_thread.isemotion = True
        else:
            self.det_thread.isemotion = False
