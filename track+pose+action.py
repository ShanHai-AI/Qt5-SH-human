...
# import some hook
import hook.skeleton_pose as skp
    ...
    for frame_idx, (path, im, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                if "pose"=="pose":
                    pose_results,vis_img=skp.skeleton_detect(outputs[i],imc)
                    ################################################
                    # 基于规则判断动作
                    ################################################
...
def parse_opt():
    ...
    parser.add_argument('--yolo-weights', nargs='+', type=Path, default=WEIGHTS / 'crowdhuman_yolov5m.pt', help='model.pt path(s)')
    parser.add_argument('--strong-sort-weights', type=Path, default=WEIGHTS / 'osnet_x0_5_msmt17.onnx')
    parser.add_argument('--config-strongsort', type=str, default='strong_sort/configs/strong_sort.yaml')
    parser.add_argument('--source', type=str, default='testvideo/V_7.mp4', help='file/dir/URL/glob, 0 for webcam')
